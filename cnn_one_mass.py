...
data = np.load('dataset/data_formatted_xyz_2ms.npy',allow_pickle=True, encoding='latin1')
name = '1 and 100 MeV simple 2 layer CNN, RBG-XYZ projections \n20x20, not normalized' #description to ass as title to figures
...
n_train = int(0.7*n_event)
n_test = int(0.3*n_event)
...
acc = np.sum(classification==labels)/len(classification) #fraction of events classified correctly


print('optimal threshold = {:.2f}, {} misclassified events ({:.2f}%), accuracy of {:.2f}, Signal acceptance of {}%'.format(thresh,n_false,n_false/n_test*100,acc*100,(n_sig-sig_false)/n_sig*100))
